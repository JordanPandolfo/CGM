...
A_aug = np.ones((np.shape(A)[0],added))
A_aug = np.append(A_aug,A,axis=1).copy()

C_aug = np.tile(np.linspace(.5,7,added),(np.shape(A)[0],1))
...
"""
Adjust Alpha Policy Function for Small Values of X
"""
A[:45,:170] = 1.0
A[45:,:50] = 1.0
...
plt.figure(1)
plt.subplot(1,2,1)
[plt.plot(x_grid,A[5*i,],label='Age %s'%str(20+5*i) ) for i in range(9)]
plt.title('Working Period')
plt.ylim(0,1)
plt.legend()
plt.subplot(1,2,2)
[plt.plot(x_grid,A[45+5*i,],label='Age %s'%str(65+5*i) ) for i in range(6)]
plt.title('Retirement Period')
plt.ylim(0,1)
plt.legend()
plt.suptitle('Portfolio Policy Functions')
#plt.savefig('policy_no.png')

plt.close()
plt.figure(2)
plt.subplot(1,2,1)
[plt.plot(x_grid,C[5*i,],label='Age %s'%str(20+5*i) ) for i in range(9)]
plt.title('Working Period')
plt.legend()
plt.ylim(0,80)
plt.subplot(1,2,2)
[plt.plot(x_grid,C[45+5*i,],label='Age %s'%str(65+5*i) ) for i in range(7)]
plt.title('Retirement Period')
plt.legend()
plt.ylim(0,80)
plt.suptitle('Consumption Policy Functions')
#plt.savefig('policy1_no.png')
#-----------------------#
#                       #
#   Model Simulation    #
#                       #
...
for sim in range(N):
    ...
    while Dead == 0:
        ...
        if Age < 45: #if working
            ...
            cash_in_hand = labor_income + wealth_income
            
            index = np.argmin(np.abs(x_grid-cash_in_hand))
            
            if cash_in_hand > xb: #If state variable exceeds model domain
            
                slope_extrap = (C[Age,xn-1]-C[Age,xn-30])/(x_grid[xn-1]-x_grid[xn-30])
                consume = C[Age,xn-1]+slope_extrap*(cash_in_hand-xb)
                
            elif cash_in_hand <= xb:
        
                consume = C[Age,index]
            
            alpha = A[Age,index]
            wealth = cash_in_hand - consume
            income_saving = labor_income - consume
            
            Record[sim,Age,0] = labor_income
            Record[sim,Age,1] = wealth_income
            Record[sim,Age,2] = cash_in_hand
